...
export function PrevisionBudgetaireWidget({ data }) {
  ...
  const variationColor = (n, n1) => {
    ...
    if (pct > 5) return COMPLIANCE_RED;
    if (pct < -5) return COMPLIANCE_GREEN;
    return '#8B9099';
  };
...
const STATUT_CONFIG = {
  en_attente: { label: 'En attente', color: COMPLIANCE_ORANGE },
  valide:     { label: 'Validé',     color: COMPLIANCE_GREEN  },
  refuse:     { label: 'Refusé',     color: COMPLIANCE_RED    },
};
