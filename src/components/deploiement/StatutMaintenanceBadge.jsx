...
// Les 5 valeurs sont celles renvoyées par l'API, dérivées des périodes de
// maintenance (server/utils/maintenanceLicence.js, #201) : active / a_venir /
// echue / arretee / aucune. Aucune n'est calculée ici.
...
export default function StatutMaintenanceBadge({ licence, compact = false }) {
  ...
  if (s === 'a_venir') return <Badge variant="neutral" label={`Maintenance à venir${licence.date_debut_maintenance ? ` le ${licence.date_debut_maintenance}` : ''}`} />;
