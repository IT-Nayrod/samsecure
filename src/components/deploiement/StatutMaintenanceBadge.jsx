...
export default function StatutMaintenanceBadge({ licence, compact = false }) {
  ...
  if (s === 'echue') return <Badge variant="warning" label={`Maintenance echue${licence.date_fin_maintenance ? ` le ${licence.date_fin_maintenance}` : ''}`} />;
  return (
    <span className="inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300">
      <ShieldOff size={11} />
      Maintenance arretee{licence.date_arret_maintenance ? ` le ${licence.date_arret_maintenance}` : ''}
      {licence.version_figee_label ? ` - version figee ${licence.version_figee_label}` : ''}
    </span>
  );
}
